...
// Lua: write( pin, level )
static int lcd_locate( lua_State* L )
{
  unsigned int x;
  unsigned int y;
...
// Module function map
static const LUA_REG_TYPE lcd_map[] = {
  { LSTRKEY( "init" ), LFUNCVAL( lcd_init ) },
  { LSTRKEY( "test" ), LFUNCVAL( test_identity ) },
  { LSTRKEY( "locate" ), LFUNCVAL( lcd_locate ) },
...
NODEMCU_MODULE(LCD, "lcd", lcd_map, luaopen_lcd);
